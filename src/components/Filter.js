...
export default function Filter({ currentCategory="image" }) {
  ...
  async function loadProducts(url) {
    ...
    let dataProduct = await fetchData(url, "", "POST").then(
      (data) => {
        setProduct([data]);
        setTimeout(
          () => {
  ...
  async function loadCategories() {
    // fetching categories
    try {
      let data = await fetchData('/api/categories', "", "POST")
      if(categories !== data) setCategories(data);
    } catch (error) {
        ...
        <div className="w-9/12">
          ...
          >
            {product &&
              ...
              product[0] &&
              product[0].status == 1 &&
              product[0].response.map((productDetails) => {
                return (
                  <Card
